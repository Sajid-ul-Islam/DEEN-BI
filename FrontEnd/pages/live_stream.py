...
def get_category(name):
    ...
    def has_any(keywords, text):
        return any(
            re.search(rf"\b{re.escape(kw.lower())}\b", text, re.IGNORECASE) for kw in keywords
        )
...
def process_data(df, selected_cols):
    ...
    try:
        ...
        if len(others) > 0:
            log_system_event(
                "OTHERS_LOG",
                {"count": len(others), "samples": others["Internal_Name"].head(10).tolist()},
            )

        summary = (
            df.groupby("Category").agg({"Internal_Qty": "sum", "Total Amount": "sum"}).reset_index()
        )
...
def _render_welcome_popup_content(summ, basket, last_updated="N/A", focus="all"):
    ...
    with st.container():
        ...
        if focus in ("all", "visual_analytics"):
            ...
            v1, v2 = st.columns(2)
            with v1:
                total_cat_rev = summ["Total Amount"].sum()
                pos_array = [
                    "auto" if (val / max(total_cat_rev, 1)) >= 0.02 else "none"
                    for val in summ["Total Amount"]
                ]
    ...
    if st.button(
        "Close & Continue to Dashboard", use_container_width=True, key=f"close_popup_{focus}"
    ):
        st.rerun()
...
def render_dashboard_output(drill, summ, top, timeframe, basket, source_name, last_updated="N/A"):
    ...
    with st.container():
        ...
        with v1:
            ...
            fig_pie.update_layout(
                ...
                showlegend=True,
                legend=dict(
                    orientation="v", yanchor="top", y=1, xanchor="left", x=1.05, font=dict(size=11)
                ),
            )
            fig_pie.update_traces(
                textposition=pos_array,
    ...
    st.plotly_chart(
        fig_top, use_container_width=True, config={"scrollZoom": True, "displayModeBar": False}
    )
...
def render_manual_tab():
    ...
    from BackEnd.services.hybrid_data_loader import get_data_summary
    ...
    if "manual_df" in st.session_state and st.session_state.manual_df is not None:
        ...
        # Show instructions
        st.info(
            "Select a date range and click Search to analyze hybrid data (historical + live 2026)."
        )
